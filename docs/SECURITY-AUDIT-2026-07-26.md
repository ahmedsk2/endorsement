...
| `SPC-RPT-058` | `DATABASE-014` · database | **FIXED** (`2026_08_15_120001_widen_rich_text_handover_columns`, `App\Rules\MaxSanitizedBytes`, `App\Casts\SanitizedHtml::MAX_PLAINTEXT_BYTES`). The four encrypted rich-text columns were not widened with the others — MySQL `TEXT` caps plaintext at 65,535 stored bytes. **Corrected arithmetic**: the ~1.4× estimate below was wrong; `Crypt::encryptString()`'s actual growth is a **measured 1.783×** (the same constant `App\Casts\EncryptedJson::MAX_BYTES` already carries), giving a true ceiling of **36,751 post-sanitize plaintext bytes** — not "near 46 KB". Worse, the request validator bounded these fields at 20,000 *characters* of the raw value while the database enforced *bytes* of the sanitized value, so the two limits silently diverged for anything wider than ASCII: 18,375 Arabic characters (2 B), 12,250 CJK/Arabic-presentation (3 B), 9,187 emoji (4 B), or as few as 7,350 plain `&` characters once the sanitizer expanded each to `&amp;` — all comfortably inside the stated 20,000-character allowance yet over the real byte ceiling, producing `SQLSTATE[22001] 1406 Data too long for column`. `strict => true` turned overflow into a visible error rather than silent truncation, and the purifier allows no `img`, so no data-URI inflation — but the failure was reachable at all, invisible to SQLite (no such limit), and worsened over time because `nevent` carries forward on every new day. | `mediumText()` on `disease`, `details`, `plan`, `nevent` (additive widening, not a retype — done). Validator now bounds POST-SANITIZE BYTES (`MaxSanitizedBytes`, 100,000-byte ceiling derived from the sanitizer's own worst-case 5× entity-expansion), not raw characters — same shape as `EncryptedJson::MAX_BYTES`. `strict => true` kept. `down()` checks `MAX(LENGTH())` per column and aborts rather than truncate a clinical field on rollback. |
