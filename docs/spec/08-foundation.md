...
**Capability catalog (complete):** `endorsement.view`, `endorsement.edit`, `endorsement.reopen`, `endorsement.compliance`, `profile.manage`, `users.manage`, `users.manage_residents`, `access.manage`, `settings.manage`, `structure.manage`, `people.manage`.

**Role defaults:** Administrator — all. Charge Nurse, Consultant, Resident — view + edit + profile.manage. Chief Resident — view + edit + profile.manage + `users.manage_residents`. Nurse (position 1, RETIRED) — no defaults. `endorsement.reopen` and `endorsement.compliance` default **Administrator-only**, grantable per role or per named user. `structure.manage` (units, levels, calendar, periods, holidays — Munawib UN-*, LV-01, ST-02) also defaults **Administrator-only**, added P1b 2026-08-09. `people.manage` (the roster: people, levels, promotion, roster import — Munawib PE-\*, LV-02…04, ST-04) also defaults **Administrator-only**, added P1c 2026-08-09. Capabilities are **global**, not unit-scoped **[RULING]** (unit-scoped keys can be added to the same catalog later without schema change).
